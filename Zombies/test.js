...
/*let result = candidate.reduce((acc, candidate) => {
	if (!acc.length) {
...
/*console.log(`${getObjectAttr(candidate)}`);

let topScore = Math.max.apply(null, candidate.map(candidate => candidate.scores));
candidate = candidate.filter(candidate => candidate.scores === topScore);
