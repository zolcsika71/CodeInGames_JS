...
class Bot  {
	...
	runner (pod0 = pods[this.id], pod1 = pods[this.id + 1]) {
		...
		//console.error(`${pod0.id} pod0: ${pod0.scores()} ${pod1.id} pod1: ${pod1.scores()}`);
...
class SearchBot extends Bot {
	...
	evaluate () {
		...
		if (myRunner.timeout === 100 || myBlocker.timeout === 100)
			...
			//scores += myBlocker.scores() - oppBlocker.scores();
			score -= myBlocker.dist(oppRunner);
			//scores -= myBlocker.dist(cps[oppRunner.ncpId]);
			//scores -= myBlocker.diffAngle(oppRunner);
		}

		//if (this.id === 2)
		//    console.error(`myRunner: ${myRunner.id} myBlocker: ${myBlocker.id} oppRunner: ${oppRunner.id} oppBlocker: ${oppBlocker.id} scores: ${scores}`);
...
while (true) {
	...
	if (!TEST_REFLEX) {
		...
		//console.error(`oppScore ${opp.solution.scores} meScore: ${me.solution.scores}`);
	}
